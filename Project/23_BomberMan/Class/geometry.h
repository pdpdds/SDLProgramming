...
inline bool GetTangentPoints (Vector2D C, double R, Vector2D P, Vector2D& T1, Vector2D& T2)
{
  ...
  T1.m_x = C.m_x + R*(R*PmC.m_x - PmC.m_y*Root)*InvSqrLen;
  T1.m_y = C.m_y + R*(R*PmC.m_y + PmC.m_x*Root)*InvSqrLen;
  T2.m_x = C.m_x + R*(R*PmC.m_x + PmC.m_y*Root)*InvSqrLen;
  T2.m_y = C.m_y + R*(R*PmC.m_y - PmC.m_x*Root)*InvSqrLen;
                                ...
                                Vector2D P)
{
  ...
  //vertex must be A
	double dotA = (P.m_x - A.m_x)*(B.m_x - A.m_x) + (P.m_y - A.m_y)*(B.m_y - A.m_y);

  if (dotA <= 0) return Vec2DDistance(A, P);

  //if the angle is obtuse between PB and AB is obtuse then the closest
  //vertex must be B
  double dotB = (P.m_x - B.m_x)*(A.m_x - B.m_x) + (P.m_y - B.m_y)*(A.m_y - B.m_y);
                        ...
                        Vector2D D)
{
	...
	double Bot = (B.m_x - A.m_x)*(D.m_y - C.m_y) - (B.m_y - A.m_y)*(D.m_x - C.m_x);
...
{
	double DistBetweenCenters = sqrt((c1.m_x - c2.m_x) * (c1.m_x - c2.m_x) +
		(c1.m_y - c2.m_y) * (c1.m_y - c2.m_y));
                                                         ...
                                                         Vector2D& IntersectionPoint)
{
  ...
  //line segment
  if ((LocalPos.m_x + radius >= 0) &&
	  ((LocalPos.m_x - radius)*(LocalPos.m_x - radius) <= Vec2DDistanceSq(B, A)))
  {
     //if the distance from the x axis to the object's position is less
     //than its radius then there is a potential intersection.
	  if (fabs(LocalPos.m_y) < radius)
     {
        ...
        //need to look at the smallest positive value of x.
		 double a = LocalPos.m_x;
		 double b = LocalPos.m_y;
