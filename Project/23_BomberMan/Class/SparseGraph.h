...
int SparseGraph<node_type, edge_type>::AddNode(int tileRow, int tileColumn)
{
	...
	int nodeIndex = m_iNextNodeIndex;
	Vector2D a;
	a.m_x = tileColumn * 32 + 16;
	a.m_y = tileRow * 32 + 16;
	NodeType NewNode(m_iNextNodeIndex, a);
	AddNode(NewNode);
	...
	for (int i = 0; i < m_Nodes.size(); i++)
	{
		...
		if (test == m_Nodes[i].Pos())
		{
			EdgeType NextEdge(NewNode.Index(), i, 1.0, 0, 0);
			AddEdge(NextEdge);
		}
...
bool SparseGraph<node_type, edge_type>::Load(std::vector<Vector2D>& vec)
{
	...
	std::vector<Vector2D>::iterator iter = vec.begin();
	
	for (iter; iter != vec.end(); iter++)
	{

		NodeType NewNode(m_iNextNodeIndex, *iter);
