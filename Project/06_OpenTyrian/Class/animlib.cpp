...
int JE_loadPage( unsigned int pagenumber )
{
	 ...
	 */
	SDL_RWseek(InFile, ANIM_OFFSET + (pagenumber * ANI_PAGE_SIZE), SEEK_SET);
	efread(InFile, &CurrentPageHeader.baseRecord, 2, 1);
	efread(InFile, &CurrentPageHeader.nRecords, 2, 1);
	efread(InFile, &CurrentPageHeader.nBytes, 2, 1);

	SDL_RWseek(InFile, 2, SEEK_CUR);
	for (i = 0; i < CurrentPageHeader.nRecords; i++)
	{
		efread(InFile,&CurrentPageRecordSizes[i], 2, 1);
	}

	/* What remains is the 'compressed' data */
	efread(InFile,CurrentPageBuffer, 1, CurrentPageHeader.nBytes);
...
int JE_loadAnim( const char *filename )
{
	...
	efread(InFile, &temp, 1, 4); /* The ID, should equal "LPF " */
	SDL_RWseek(InFile, 2, SEEK_CUR); /* skip over this word */
	efread(InFile, &FileHeader.nlps, 2, 1); /* Number of pages */
	efread(InFile, &FileHeader.nRecords, 4, 1); /* Number of records */
	...
	/* Read in headers */
	SDL_RWseek(InFile, PAGEHEADER_OFFSET, SEEK_SET);
	for (i = 0; i < FileHeader.nlps; i++)
	{
		efread(InFile, &PageHeader[i].baseRecord, 2, 1);
		efread(InFile, &PageHeader[i].nRecords, 2, 1);
		efread(InFile, &PageHeader[i].nBytes, 2, 1);
	}
	...
	SDL_RWseek(InFile, PALETTE_OFFSET, SEEK_SET);
	for (i = 0; i < 256; i++)
	{
		efread(InFile, &colors[i].b, 1, 1);
		efread(InFile, &colors[i].g, 1, 1);
		efread(InFile, &colors[i].r, 1, 1);
		efread(InFile, &colors[i].a, 1, 1);
	}
