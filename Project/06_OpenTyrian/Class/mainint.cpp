...
void JE_nextEpisode( void )
{
	...
	if (episodeNum == initial_episode_num && !gameHasRepeated && episodeNum != 4 &&
...
{
	bool temp = keysactive[SDL_GetScancodeFromKey(SDLK_F11)] != 0;
	if (temp)
	{
		keysactive[SDL_GetScancodeFromKey(SDLK_F11)] = false;
		newkey = false;
...
bool load_next_demo( void )
{
	...
	JE_initEpisode(temp);
	efread(demo_file, levelName, 1, 10); levelName[10] = '\0';
...
bool read_demo_keys( void )
{
	...
	efread(demo_file, &demo_keys_wait, sizeof(Uint16), 1);
	demo_keys_wait = SDL_Swap16(demo_keys_wait);

	next_demo_keys = egetc(demo_file);
...
void JE_playCredits( void )
{
	...
	// load credits text
	SDL_RWops *f = dir_fopen_die(data_dir(), "tyrian.cdt", "rb");
...
void JE_mainKeyboardInput( void )
{
	...
	/* { In-Game Help } */
	if (keysactive[SDL_GetScancodeFromKey(SDLK_F1)])
	{
		if (isNetworkGame)
		{
			helpRequest = true;
	...
	/* {!Activate Nort Ship!} */
	if (keysactive[SDL_GetScancodeFromKey(SDLK_F2)] && keysactive[SDL_GetScancodeFromKey(SDLK_F4)] && keysactive[SDL_GetScancodeFromKey(SDLK_F6)] && keysactive[SDL_GetScancodeFromKey(SDLK_F7)] &&
		keysactive[SDL_GetScancodeFromKey(SDLK_F9)] && keysactive[SDL_GetScancodeFromKey(SDLK_BACKSLASH)] && keysactive[SDL_GetScancodeFromKey(SDLK_SLASH)])
	{
		if (isNetworkGame)
		{
	...
	{
		if (keysactive[SDL_GetScancodeFromKey(SDLK_F2)] && keysactive[SDL_GetScancodeFromKey(SDLK_F3)] && keysactive[SDL_GetScancodeFromKey(SDLK_F6)])
		{
			youAreCheating = !youAreCheating;
			keysactive[SDL_GetScancodeFromKey(SDLK_F2)] = false;
		}

		if (keysactive[SDL_GetScancodeFromKey(SDLK_F2)] && keysactive[SDL_GetScancodeFromKey(SDLK_F3)] && 
			(keysactive[SDL_GetScancodeFromKey(SDLK_F4)] || keysactive[SDL_GetScancodeFromKey(SDLK_F5)]) && !superTyrian)
		{
			for (uint i = 0; i < COUNTOF(player); ++i)
				player[i].armor = 0;
		...
		if (constantPlay && keysactive[SDL_GetScancodeFromKey(SDLK_c)] && !superTyrian && superArcadeMode == SA_NONE)
		{
			youAreCheating = !youAreCheating;
			keysactive[SDL_GetScancodeFromKey(SDLK_c)] = false;
		}
	}
	...
	/* {DEBUG} */
	if (keysactive[SDL_GetScancodeFromKey(SDLK_F10)] && keysactive[SDL_GetScancodeFromKey(SDLK_BACKSPACE)])
	{
		keysactive[SDL_GetScancodeFromKey(SDLK_F10)] = false;
		debug = !debug;
	...
	/* {CHEAT-SKIP LEVEL} */
	if (keysactive[SDL_GetScancodeFromKey(SDLK_F2)] && keysactive[SDL_GetScancodeFromKey(SDLK_F6)] && (keysactive[SDL_GetScancodeFromKey(SDLK_F7)] || 
		keysactive[SDL_GetScancodeFromKey(SDLK_F8)]) && !keysactive[SDL_GetScancodeFromKey(SDLK_F9)]
	...
	/* pause game */
	pause_pressed = pause_pressed || keysactive[SDL_GetScancodeFromKey(SDLK_p)] || keysactive[SDL_GetScancodeFromKey(SDLK_PAUSE)];

	/* in-game setup */
	ingamemenu_pressed = ingamemenu_pressed || keysactive[SDL_GetScancodeFromKey(SDLK_ESCAPE)];

	/* {MUTE SOUND} */
	if (keysactive[SDL_GetScancodeFromKey(SDLK_s)])
	{
		keysactive[SDL_GetScancodeFromKey(SDLK_s)] = false;
	...
	/* {MUTE MUSIC} */
	if (keysactive[SDL_GetScancodeFromKey(SDLK_m)])
	{
		keysactive[SDL_GetScancodeFromKey(SDLK_m)] = false;
	...
	if (keysactive[SDL_GetScancodeFromKey(SDLK_BACKSPACE)])
	{
		/* toggle screenshot pause */
		if (keysactive[SDL_GetScancodeFromKey(SDLK_NUMLOCKCLEAR)])
		{
			superPause = !superPause;
		}

		/* {SMOOTHIES} */
		if (keysactive[SDL_GetScancodeFromKey(SDLK_F12)] && keysactive[SDL_GetScancodeFromKey(SDLK_SCROLLLOCK)])
		{
			...
			if (keysactive[SDL_GetScancodeFromKey(SDLK_0)])
			{
		...
		if (keysactive[SDL_GetScancodeFromKey(SDLK_MINUS)])
		{
			if (levelFilter == -99)
			{
				levelFilter = 0;
		...
		/* {HYPER-SPEED} */
		if (keysactive[SDL_GetScancodeFromKey(SDLK_1)])
		{
			fastPlay++;
			if (fastPlay > 2)
			{
			...
			keysactive[SDL_GetScancodeFromKey(SDLK_1)] = false;
			JE_setNewGameSpeed();
		}

		/* {IN-GAME RANDOM MUSIC SELECTION} */
		if (keysactive[SDL_GetScancodeFromKey(SDLK_SCROLLLOCK)])
		{
		...
		if (!isNetworkGame || playerNum_ == thisPlayerNum)
		{
			...
			else
			{
				...
				if ((inputDevice == 0 || inputDevice == 1 || inputDevice == 2) && !play_demo)
				{
					...
					if (keysactive[SDL_GetScancodeFromKey(keySettings[0])] ||
					...
					if (keysactive[SDL_GetScancodeFromKey(keySettings[1])] ||
						(has_mouse && mouse_pressed[0] && (shifted_mouse_y > this_player->y && touchscreenMode < TOUCHSCREEN_FIRE_ONLY)))
						this_player->y += CURRENT_KEY_SPEED;

					if (keysactive[SDL_GetScancodeFromKey(keySettings[2])] ||
					...
					if (keysactive[SDL_GetScancodeFromKey(keySettings[3])] ||
						(has_mouse && mouse_pressed[0] && (shifted_mouse_x > this_player->x && touchscreenMode < TOUCHSCREEN_FIRE_ONLY)))
						this_player->x += CURRENT_KEY_SPEED;
					...
					button[3] = button[3] || keysactive[SDL_GetScancodeFromKey(keySettings[5])];
					button[1] = button[1] || keysactive[SDL_GetScancodeFromKey(keySettings[6])];
					button[2] = button[2] || keysactive[SDL_GetScancodeFromKey(keySettings[7])];
					...
					if (record_demo)
					{
						...
						{
							demo_keys_wait = SDL_Swap16(demo_keys_wait);
							efwrite(demo_file, &demo_keys_wait, sizeof(Uint16), 1);
						...
						if (this_option->wport > 0)
						{
							...
							else
							{
								...
								if (ammo_max > 0)  // sidekick has limited ammo
								{
									...
									{
										JE_initPlayerShot(this_option->wport, shot_i, this_player->sidekick[i].x, this_player->sidekick[i].y, *mouseX_, *mouseY_, this_option->wpnum + this_player->sidekick[i].charge, playerNum_);
