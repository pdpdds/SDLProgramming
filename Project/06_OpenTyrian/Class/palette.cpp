...
{
	SDL_RWops *f = dir_fopen_die(data_dir(), "palette.dat", "rb");
	...
	for (int p = 0; p < palette_count; ++p)
	{
		for (int i = 0; i < 256; ++i)
		{
			palettes[p][i].r = egetc(f) << 2;
			palettes[p][i].g = egetc(f) << 2;
			palettes[p][i].b = egetc(f) << 2;
		}
	}
...
void set_palette( Palette colors, unsigned int first_color, unsigned int last_color )
{
	...
	if (bpp == 8)
		SDL_SetPaletteColors(surface->format->palette, palette, first_color, last_color - first_color + 1);
}
...
void step_fade_palette( int diff[256][3], int steps, unsigned int first_color, unsigned int last_color )
{
	...
	if (bpp == 8)
		SDL_SetPaletteColors(surface->format->palette, &palette[0], 0, 1);
}
