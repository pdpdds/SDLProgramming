...
start_level_first:
	...
	if (!play_demo && record_demo)
	{
		...
		efwrite(demo_file, &episodeNum, 1, 1);
		efwrite(demo_file, levelName, 1, 10);
		efwrite(demo_file, &lvlFileNum, 1, 1);
		...
		efwrite(demo_file, &levelSong, 1, 1);
...
new_game:
	...
	if (!play_demo)
	{
		do
		{
			SDL_RWops *ep_f = dir_fopen_die(data_dir(), episode_file, "rb");
	...
	SDL_RWops *level_f = dir_fopen_die(data_dir(), levelFile, "rb");
	SDL_RWseek(level_f, lvlPos[(lvlFileNum - 1) * 2], SEEK_SET);

	efgetc(level_f); // char_mapFile
	JE_char char_shapeFile = efgetc(level_f);
	efread(level_f, &mapX, sizeof(JE_word), 1);
	efread(level_f, &mapX2, sizeof(JE_word), 1);
	efread(level_f, &mapX3, sizeof(JE_word), 1);

	efread(level_f, &levelEnemyMax, sizeof(JE_word), 1);
	for (x = 0; x < levelEnemyMax; x++)
	{
		efread(level_f, &levelEnemy[x], sizeof(JE_word), 1);
	}

	efread(level_f, &maxEvent, sizeof(JE_word), 1);
	for (x = 0; x < maxEvent; x++)
	{
		efread(level_f, &eventRec[x].eventtime, sizeof(JE_word), 1);
		efread(level_f, &eventRec[x].eventtype, sizeof(JE_byte), 1);
		efread(level_f, &eventRec[x].eventdat, sizeof(JE_integer), 1);
		efread(level_f, &eventRec[x].eventdat2, sizeof(JE_integer), 1);
		efread(level_f, &eventRec[x].eventdat3, sizeof(JE_shortint), 1);
		efread(level_f, &eventRec[x].eventdat5, sizeof(JE_shortint), 1);
		efread(level_f, &eventRec[x].eventdat6, sizeof(JE_shortint), 1);
		efread(level_f, &eventRec[x].eventdat4, sizeof(JE_byte), 1);
	}
	...
	efread(level_f, mapSh, sizeof(JE_word), sizeof(mapSh) / sizeof(JE_word));
	for (temp = 0; temp < 3; temp++)
	{
	...
	SDL_RWops *shpFile = dir_fopen_die(data_dir(), tempStr, "rb");
	...
	for (int z = 0; z < 600; z++)
	{
		...
		if (shapeBlank)
			...
			efread(shpFile, shape, sizeof(JE_byte), sizeof(shape));
	...
	efread(level_f, mapBuf, sizeof(JE_byte), 14 * 300);
	...
	efread(level_f, mapBuf, sizeof(JE_byte), 14 * 600 );
	...
	efread(level_f, mapBuf, sizeof(JE_byte), 15 * 600);
			...
			for (unsigned int i = 0; i < SA_ENGAGE; i++)
			{
				...
				if (arcade_code_i[i] > 0 && arcade_code_i[i] == strlen(specialName[i]))
				{
					...
					else if (i+1 == SA_ENGAGE)
					{
						...
						initialDifficulty = keysactive[SDLK_SCROLLLOCK] ? 6 : 8;
