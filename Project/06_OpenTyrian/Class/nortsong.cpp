...
void JE_loadSndFile( const char *effects_sndfile, const char *voices_sndfile )
{
	...
	efread(fi, &sndNum, sizeof(sndNum), 1);

	for (x = 0; x < sndNum; x++)
	{
		efread(fi, &sndPos[0][x], sizeof(sndPos[0][x]), 1);
	}
	...
	for (z = 0; z < sndNum; z++)
	{
		...
		efread(fi, digiFx[z], 1, fxSize[z]); /* JE: Load sample to buffer */
	}
	...
	for (x = 0; x < sndNum; x++)
	{
		efread(fi, &sndPos[1][x], sizeof(sndPos[1][x]), 1);
	}
	efseek(fi, 0, SEEK_END);
	sndPos[1][sndNum] = eftell(fi); /* Store file size */
	...
	for (y = 0; y < sndNum; y++)
	{
		...
		efread(fi, digiFx[z + y], 1, fxSize[z + y]); /* JE: Load sample to buffer */
	}
