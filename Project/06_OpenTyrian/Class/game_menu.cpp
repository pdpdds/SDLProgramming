			...
			/* Inner loop -- this handles animations on menus that need them and handles
			   ...
			   Also, I think all timing is handled in here. Somehow. */
				...
				if (curMenu == 8)
				{
					...
					if (keysactive[SDL_GetScancodeFromKey(SDLK_PAGEUP)])
					{
						yChg = -2;
						inputDetected = false;
					}
					if (keysactive[SDL_GetScancodeFromKey(SDLK_PAGEDOWN)])
					{
						yChg = 2;
						inputDetected = false;
					}
					...
					if (keysactive[SDL_GetScancodeFromKey(SDLK_UP)] || keysactive[SDL_GetScancodeFromKey(SDLK_LCTRL)] || joystick_up)
					{
						yChg = -1;
						inputDetected = false;
					}

					if (keysactive[SDL_GetScancodeFromKey(SDLK_DOWN)] || keysactive[SDL_GetScancodeFromKey(SDLK_LALT)] || joystick_down)
					{
						yChg = 1;
						inputDetected = false;
					}
...
{
	SDL_RWops *f = dir_fopen_die(data_dir(), cube_file, "rb");
