...
void load_sprites_file( unsigned int table, const char *filename )
{
	...
	SDL_RWops *f = dir_fopen_die(data_dir(), filename, "rb");
...
void load_sprites(unsigned int table, SDL_RWops *f)
{
	free_sprites(table);
	
	Uint16 temp;
	efread(f, &temp, sizeof(Uint16), 1);
	...
	for (unsigned int i = 0; i < sprite_table[table].count; ++i)
	{
		...
		if (!egetc(f)) // sprite is empty
			continue;
		
		efread(f, &cur_sprite->width,  sizeof(Uint16), 1);
		efread(f, &cur_sprite->height, sizeof(Uint16), 1);
		efread(f, &cur_sprite->size, sizeof(Uint16), 1);
		
		cur_sprite->data = (Uint8 *)malloc(cur_sprite->size);
		
		efread(f, cur_sprite->data, sizeof(Uint8), cur_sprite->size);
	}
}
...
void JE_loadCompShapes( Sprite2_array *sprite2s, char s )
{
	...
	SDL_RWops *f = dir_fopen_die(data_dir(), buffer, "rb");
...
void JE_loadCompShapesB(Sprite2_array *sprite2s, SDL_RWops *f)
{
	free_sprite2s(sprite2s);
	
	sprite2s->data = (Uint8 *)malloc(sizeof(Uint8) * sprite2s->size);
	efread(f, sprite2s->data, sizeof(Uint8), sprite2s->size);
}
...
void JE_loadMainShapeTables( const char *shpfile )
{
	...
	SDL_RWops *f = dir_fopen_die(data_dir(), shpfile, "rb");
	
	JE_word shpNumb;
	JE_longint shpPos[SHP_NUM + 1]; // +1 for storing file length
	
	efread(f, &shpNumb, sizeof(JE_word), 1);
	assert(shpNumb + 1u <= COUNTOF(shpPos));
	
	for (int i = 0; i < shpNumb; i++)
	{
		efread(f, &shpPos[i], sizeof(JE_longint), 1);
	}
	efseek(f, 0, SEEK_END);
	shpPos[shpNumb] = eftell(f);
