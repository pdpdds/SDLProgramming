...
void Stage02Scene::Init(Game* owner)
{
	...
	wall = new Wall("stage02.txt");	///< 벽 생성
...
void Stage02Scene::Execute(Game* owner)
{
	...
	{
		owner->FSM()->ChangeState( GameOverScene::Shaerd() ); ///< 다음 스테이지가 없어서 게임종료
		return;
	}

	if( snake->IsArrive() == false ) ///< 뱀이 죽으면
	{
		owner->SetLife( owner->Life()-1 ); ///< life 감소
		int life = owner->Life();
