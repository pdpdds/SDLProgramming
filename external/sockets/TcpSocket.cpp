...
int  TcpSocket::Open2(const std::string &host,port_t port,char *szTemp)
{
...
#endif
//		sprintf(szTemp, "[%d - %d - %d]", ad.GetFamily(), port, l);
		
		// if not, create new connection
//		SOCKET s = CreateSocket(ad.GetFamily(), SOCK_STREAM, "tcp");
		SOCKET s = CreateSocket2(ad.GetFamily(), SOCK_STREAM, "tcp", szTemp);
		if (s == INVALID_SOCKET)
		{
			return 5;
		}
